...
export class NFSClientAdapter implements EggObjectLifecycle, NFSClient {
  ...
  async appendBuffer(bytes: Uint8Array, options: UploadOptions): Promise<AppendResult> {
  ...
  async uploadBuffer(bytes: Uint8Array, options: UploadOptions): Promise<UploadResult> {
    if (this._client.uploadBytes) {
      return await this._client.uploadBytes(bytes, options);
    }
    return await this._client.uploadBuffer(bytes, options);
